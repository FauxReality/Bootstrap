...
export default function App(){
  ...
  {/* Header: Brand left + right column shown only in PDF */}
  ...
  <div className="pdf-header">
    ...
    <div className="pdf-right">
      ...
      <div className="font-medium" style={{marginTop:12}}>Method</div>
      <div>{payment?.method || invoice?.method || invoice?.paymentMethod || '—'}</div>

      <div className="font-medium" style={{marginTop:12}}>Contact</div>
  ...
  {/* Info grids — hide duplicates in PDF */}
  <div className="grid md:grid-cols-2 gap-4 mb-6">
    <RO label="Invoice #"    val={String(invoice.invoiceNumber)} />
    <RO label="Payment Date" val={payment.date || invoice.paymentDate || invoice.date} className="pdf-hide" />
    <RO label="Service Day"  val={wday(reg.serviceDate)} />
    <RO label="Method"       val={payment.method || invoice.method || invoice.paymentMethod} className="pdf-hide" />
    {payment.reference && <RO label="Reference" val={payment.reference} />}
  </div>

  <div className="grid md:grid-cols-2 gap-4 mb-6">
    <RO label="Received From" val={`${reg.firstName} ${reg.lastName}`} />
    <RO label="Contact"       val={`${reg.phone} • ${reg.email}`} className="pdf-hide" />
...
const Brand = ({ biz, nameClass = "text-2xl", metaClass = "text-lg" }) => {
  ...
  return (
    <div className="brand flex flex-col md:flex-row print:flex-row items-start md:items-center print:items-center gap-4 mb-4 text-left">
      <div>
        {biz?.logoUrl ? (
