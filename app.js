...
export default function App(){
  ...
  <div className="print:flex print:justify-between print:items-start print:gap-6">
    ...
    <div className="hidden print:block w-64 text-sm">
      ...
      <div>{
        (Array.isArray(reg?.services) && reg.services.length)
          ? reg.services.join(', ')
          : '—'
      }</div>
    </div>
  ...
  <h2 className="text-lg font-semibold mb-4 text-center print:text-left">Payment Receipt</h2>
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-6">
        ...
        <RO label="Payment Date" val={payment.date} className="print:hidden" />
    ...
    <div className="flex flex-wrap gap-2 justify-end mt-6">
      <button
        onClick={() => openPdfFromEl(document.getElementById('receipt-print'), `Receipt-${invoice?.invoiceNumber ?? ''}`)}
        className="rounded-xl px-4 py-2 bg-gray-900 text-white hover:bg-black"
      >
        Download PDF
      </button>
      <button onClick={() => setPage(2)} className="rounded-xl px-4 py-2 bg-gray-200 hover:bg-gray-300">
        Back to Invoice
      </button>
    </div>
  </section>
